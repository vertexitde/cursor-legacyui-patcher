<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 2.7.0-pre.119 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 2.7.0-pre.119.
    pre.119 variables: Fb_ (2 layouts), wh (clear), zD (layoutEnum), nM (cmd), Hb_ (createInstance), bi (StorageScope), xI (openerCheck).
    Note: Uses S() for i18n, getSidebarLocation() in renderModeGrid.
    NLS IDs: Agent=3027, Editor=3028, Settings=3024, Zen=3268, Browser=3269.
...
$Patches = @(
    ...
    @{
        ...
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>q,anchorAlignment:1,anchorPosition:0,layer:-24,render:$=>{try{const G=xI(a,u)?_:void 0;return i.createInstance(Hb_'
        Replace = '{getAnchor:()=>q,anchorAlignment:1,anchorPosition:0,render:$=>{try{const G=xI(a,u)?_:void 0;return i.createInstance(Hb_'
    },
    @{
        Name                = "buildContent_full_pre119"
        AllowLengthMismatch = $true
    ...
    @{
        Name                = "appendCursorSettingsButton_keybinding_pre119"
        AllowLengthMismatch = $true
        Search  = 'appendCursorSettingsButton(n){const e=document.createElement("button");e.type="button",e.className="agent-layout-quick-menu__footer-link";const t=document.createElement("span");t.className="agent-layout-quick-menu__label",t.textContent=S(3024,null),e.appendChild(t),e.addEventListener("click",i=>{i.stopPropagation(),this.commandService.executeCommand(nM).finally(()=>this.onRequestClose())}),n.appendChild(e)}'
        Replace = 'appendCursorSettingsButton(n){const e=document.createElement("button");e.type="button",e.className="agent-layout-quick-menu__footer-link";const t=document.createElement("span");t.className="agent-layout-quick-menu__label",t.textContent="Cursor Settings",e.appendChild(t);const i=this.getKeybindingLabel(nM);if(i){const r=document.createElement("span");r.className="agent-layout-quick-menu__keybinding",r.textContent=i,e.appendChild(r)}e.addEventListener("click",o=>{o.stopPropagation(),this.commandService.executeCommand(nM).finally(()=>this.onRequestClose())}),n.appendChild(e)}'
    }
)
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (2.7.0-pre.119) ===" "Cyan"
