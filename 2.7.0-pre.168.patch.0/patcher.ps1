<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 2.7.0-pre.168 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 2.7.0-pre.168.
    pre.168 variables: gP_ (2 layouts), Ih (clear), F2 (layoutEnum), LM (cmd), yP_ (createInstance), bi (StorageScope), openerCheck nB.
    Zen C(3190)=Zen Mode (not C(3069) Layout+Zen). Browser C(3070). Agent/Editor C(3048)/C(3049). Settings footer C(3045).
    Stock buildContent: glass before settings; expanded Replace ends with appendCursorSettingsButton then appendGlassWindowButton.
    Opener: getAnchor ()=>$, render U=>, createInstance yP_.
#>
...
$Patches = @(
    ...
    @{
        ...
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>$,anchorAlignment:1,anchorPosition:0,layer:-24,render:U=>{try{const G=nB(a,u)?_:void 0;return i.createInstance(yP_'
        Replace = '{getAnchor:()=>$,anchorAlignment:1,anchorPosition:0,render:U=>{try{const G=nB(a,u)?_:void 0;return i.createInstance(yP_'
    },
    @{
        Name                = "buildContent_full_pre168"
        AllowLengthMismatch = $true
    ...
    @{
        Name                = "appendCursorSettingsButton_keybinding_pre168"
        AllowLengthMismatch = $true
        Search  = 'appendCursorSettingsButton(n){const e=document.createElement("button");e.type="button",e.className="agent-layout-quick-menu__footer-link";const t=document.createElement("span");t.className="agent-layout-quick-menu__label",t.textContent=C(3045,null),e.appendChild(t),e.addEventListener("click",i=>{i.stopPropagation(),this.commandService.executeCommand(LM).finally(()=>this.onRequestClose())}),n.appendChild(e)}'
        Replace = 'appendCursorSettingsButton(n){const e=document.createElement("button");e.type="button",e.className="agent-layout-quick-menu__footer-link";const t=document.createElement("span");t.className="agent-layout-quick-menu__label",t.textContent="Cursor Settings",e.appendChild(t);const i=this.getKeybindingLabel(LM);if(i){const r=document.createElement("span");r.className="agent-layout-quick-menu__keybinding",r.textContent=i,e.appendChild(r)}e.addEventListener("click",o=>{o.stopPropagation(),this.commandService.executeCommand(LM).finally(()=>this.onRequestClose())}),n.appendChild(e)}'
    }
)
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (2.7.0-pre.168) ===" "Cyan"
