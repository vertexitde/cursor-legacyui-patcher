<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 3.1.0-pre.25 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 3.1.0-pre.25.
    i18n E(). cFS (layouts), Mh (clear), ZB (layoutEnum), nB (settings cmd), wi (StorageScope), opener AR + pFS.
    Zen "Zen"; Browser E(2964) ("Browser"). Agent/Editor E(2942)/E(2943). Footer stock E(2939) "Settings" (patched to label + keybinding).
    Opener: layer:-24 removed; createInstance(pFS,...).render(U); tail N,x. nls indices shift vs pre.21 — verify against your nls.messages.json.
    Add-layout tile: !isApplyingLayout && under cap. cursor-old uses different E(n) strings at the same index — do not copy IDs from an old install.
#>
...
$Patches = @(
    ...
    @{
        Name                = "opener_no_layer_325p25"
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>O,anchorAlignment:1,anchorPosition:0,layer:-24,render:U=>{try{const q=AR(a,u)?S:void 0;return i.createInstance(pFS,s,o,a,l,u,d,h,f,p,g,i,_,q,N,x).render(U)}catch(q){throw q}}'
        Replace = '{getAnchor:()=>O,anchorAlignment:1,anchorPosition:0,render:U=>{try{const q=AR(a,u)?S:void 0;return i.createInstance(pFS,s,o,a,l,u,d,h,f,p,g,i,_,q,N,x).render(U)}catch(q){throw q}}'
    },
    @{
        Name                = "buildContent_full_325p25"
        AllowLengthMismatch = $true
    ...
    @{
        Name                = "appendCursorSettingsButton_keybinding_325p25"
        AllowLengthMismatch = $true
        Search  = 'appendCursorSettingsButton(n){const e=document.createElement("button");e.type="button",e.className="agent-layout-quick-menu__footer-link";const t=document.createElement("span");t.className="agent-layout-quick-menu__label",t.textContent=E(2939,null),e.appendChild(t),e.addEventListener("click",i=>{i.stopPropagation(),this.commandService.executeCommand(nB).finally(()=>this.onRequestClose())}),n.appendChild(e)}'
        Replace = 'appendCursorSettingsButton(n){const e=document.createElement("button");e.type="button",e.className="agent-layout-quick-menu__footer-link";const t=document.createElement("span");t.className="agent-layout-quick-menu__label",t.textContent="Cursor Settings",e.appendChild(t);const i=this.getKeybindingLabel(nB);if(i){const r=document.createElement("span");r.className="agent-layout-quick-menu__keybinding",r.textContent=i,e.appendChild(r)}e.addEventListener("click",o=>{o.stopPropagation(),this.commandService.executeCommand(nB).finally(()=>this.onRequestClose())}),n.appendChild(e)}'
    }
)
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (3.1.0-pre.25) ===" "Cyan"
