<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 2.7.0-pre.16 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 2.7.0-pre.16.
    Pre.16 has only aDw (2 layouts); we inject Zen+Browser via spread. Uses rh, C(, xC,
    u4/hDw (opener), nL (Cursor Settings cmd), Si (StorageScope).
#>
...
$Patches = @(
    ...
    @{
        ...
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>$,anchorAlignment:1,anchorPosition:0,layer:-24,render:H=>{try{const G=u4(a,u)?_:void 0;return i.createInstance(hDw'
        Replace = '{getAnchor:()=>$,anchorAlignment:1,anchorPosition:0,render:H=>{try{const G=u4(a,u)?_:void 0;return i.createInstance(hDw'
    },
    @{
        Name                = "buildContent_full_pre16"
        AllowLengthMismatch = $true
    ...
    @{
        Name                = "appendCursorSettingsButton_keybinding_pre16"
        AllowLengthMismatch = $true
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (2.7.0-pre.16) ===" "Cyan"
