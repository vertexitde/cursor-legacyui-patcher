<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 2.7.0-pre.113 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 2.7.0-pre.113.
    pre.113 variables: Ym_ (2 layouts), bh (clear), $D (layoutEnum), JN (cmd), np_ (createInstance), yi (StorageScope), AI (openerCheck).
    Note: Uses C() for i18n, getSidebarLocation() in renderModeGrid.
    NLS IDs: Agent=3026, Editor=3027, Settings=3023, Zen=3267, Browser=3268.
    ...
    appendGlassWindowButton(n) last so "Switch to Glass Window" / glass UI stays available.
...
$Patches = @(
    ...
    @{
        ...
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>q,anchorAlignment:1,anchorPosition:0,layer:-24,render:$=>{try{const G=AI(a,u)?_:void 0;return i.createInstance(np_'
        Replace = '{getAnchor:()=>q,anchorAlignment:1,anchorPosition:0,render:$=>{try{const G=AI(a,u)?_:void 0;return i.createInstance(np_'
    },
    @{
        Name                = "buildContent_full_pre113"
        AllowLengthMismatch = $true
    ...
    @{
        Name                = "appendCursorSettingsButton_keybinding_pre113"
        AllowLengthMismatch = $true
        Search  = 'appendCursorSettingsButton(n){const e=document.createElement("button");e.type="button",e.className="agent-layout-quick-menu__footer-link";const t=document.createElement("span");t.className="agent-layout-quick-menu__label",t.textContent=C(3023,null),e.appendChild(t),e.addEventListener("click",i=>{i.stopPropagation(),this.commandService.executeCommand(JN).finally(()=>this.onRequestClose())}),n.appendChild(e)}'
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (2.7.0-pre.113) ===" "Cyan"
