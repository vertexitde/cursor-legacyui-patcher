<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 2.7.0-pre.84 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 2.7.0-pre.84.
    pre.84 variables: bi_ (2 layouts), mh (clear), kD (layoutEnum), IN (cmd), _i_ (createInstance), ki (StorageScope), JR (openerCheck).
    Note: Uses C() for i18n, getSidebarLocation() in renderModeGrid.
    NLS IDs: Agent=2956, Editor=2957, Settings=2955, Zen=3197, Browser=3198.
...
$Patches = @(
    ...
    @{
        ...
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>q,anchorAlignment:1,anchorPosition:0,layer:-24,render:$=>{try{const G=JR(a,u)?_:void 0;return i.createInstance(_i_'
        Replace = '{getAnchor:()=>q,anchorAlignment:1,anchorPosition:0,render:$=>{try{const G=JR(a,u)?_:void 0;return i.createInstance(_i_'
    },
    @{
        Name                = "buildContent_full_pre84"
        AllowLengthMismatch = $true
    ...
    @{
        Name                = "appendCursorSettingsButton_keybinding_pre84"
        AllowLengthMismatch = $true
        Search  = 'appendCursorSettingsButton(n){const e=document.createElement("button");e.type="button",e.className="agent-layout-quick-menu__footer-link";const t=document.createElement("span");t.className="agent-layout-quick-menu__label",t.textContent=C(2955,null),e.appendChild(t),e.addEventListener("click",i=>{i.stopPropagation(),this.commandService.executeCommand(IN).finally(()=>this.onRequestClose())}),n.appendChild(e)}'
        Replace = 'appendCursorSettingsButton(n){const e=document.createElement("button");e.type="button",e.className="agent-layout-quick-menu__footer-link";const t=document.createElement("span");t.className="agent-layout-quick-menu__label",t.textContent="Cursor Settings",e.appendChild(t);const i=this.getKeybindingLabel(IN);if(i){const r=document.createElement("span");r.className="agent-layout-quick-menu__keybinding",r.textContent=i,e.appendChild(r)}e.addEventListener("click",o=>{o.stopPropagation(),this.commandService.executeCommand(IN).finally(()=>this.onRequestClose())}),n.appendChild(e)}'
    }
)
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (2.7.0-pre.84) ===" "Cyan"
