<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 2.7.0-pre.172 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 2.7.0-pre.172.
    pre.172: i18n is E() (not C()). S9_ (layouts), Th (clear), QM (layoutEnum), jN (cmd), T9_ (createInstance), _i (StorageScope), opener uD.
    Zen E(3194), Browser E(3074); avoid E(3073) Layout+Zen. Agent/Editor E(3052)/E(3053). Settings E(3049).
    Opener: getAnchor ()=>O, render U=>, const q=uD, createInstance T9_ (removes layer:-24).
#>
...
$Patches = @(
    ...
    @{
        ...
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>O,anchorAlignment:1,anchorPosition:0,layer:-24,render:U=>{try{const q=uD(a,u)?w:void 0;return i.createInstance(T9_'
        Replace = '{getAnchor:()=>O,anchorAlignment:1,anchorPosition:0,render:U=>{try{const q=uD(a,u)?w:void 0;return i.createInstance(T9_'
    },
    @{
        Name                = "buildContent_full_pre172"
        AllowLengthMismatch = $true
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (2.7.0-pre.172) ===" "Cyan"
