<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 3.2.0-pre.24 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 3.2.0-pre.24.
    i18n E(). zJS (layouts), Mh (clear), rF (layoutEnum), sB (settings cmd), fi (StorageScope), opener MR + XJS.
    Zen "Zen"; Browser E(2964). Agent/Editor E(2942)/E(2943). Footer E(2939) → patched "Cursor Settings" + keybinding.
    Opener: layer:-24 removed; createInstance(XJS,...).render(U); tail N,x. Does not modify nls.messages.json.
...
$Patches = @(
    ...
    @{
        Name                = "opener_no_layer_320p24"
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>O,anchorAlignment:1,anchorPosition:0,layer:-24,render:U=>{try{const q=MR(a,u)?S:void 0;return i.createInstance(XJS,s,o,a,l,u,d,h,f,p,g,i,_,q,N,x).render(U)}catch(q){throw q}}'
        Replace = '{getAnchor:()=>O,anchorAlignment:1,anchorPosition:0,render:U=>{try{const q=MR(a,u)?S:void 0;return i.createInstance(XJS,s,o,a,l,u,d,h,f,p,g,i,_,q,N,x).render(U)}catch(q){throw q}}'
    },
    @{
        Name                = "buildContent_full_320p24"
        AllowLengthMismatch = $true
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (3.2.0-pre.24) ===" "Cyan"
