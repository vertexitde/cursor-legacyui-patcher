<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 2.7.0-pre.180 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 2.7.0-pre.180.
    i18n E(). J1S (layouts), Ih (clear), KB (layoutEnum), X2 (cmd), z1S (createInstance), Si (StorageScope), opener CR.
    Zen E(3205), Browser E(3085); avoid E(3084) Layout+Zen. Agent/Editor E(3063)/E(3064). Settings E(3060).
    Opener: getAnchor ()=>O, const q=CR, createInstance z1S (removes layer:-24).
#>
...
$Patches = @(
    ...
    @{
        ...
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>O,anchorAlignment:1,anchorPosition:0,layer:-24,render:U=>{try{const q=CR(a,u)?w:void 0;return i.createInstance(z1S'
        Replace = '{getAnchor:()=>O,anchorAlignment:1,anchorPosition:0,render:U=>{try{const q=CR(a,u)?w:void 0;return i.createInstance(z1S'
    },
    @{
        Name                = "buildContent_full_pre180"
        AllowLengthMismatch = $true
    ...
    @{
        Name                = "appendCursorSettingsButton_keybinding_pre180"
        AllowLengthMismatch = $true
        Search  = 'appendCursorSettingsButton(n){const e=document.createElement("button");e.type="button",e.className="agent-layout-quick-menu__footer-link";const t=document.createElement("span");t.className="agent-layout-quick-menu__label",t.textContent=E(3060,null),e.appendChild(t),e.addEventListener("click",i=>{i.stopPropagation(),this.commandService.executeCommand(X2).finally(()=>this.onRequestClose())}),n.appendChild(e)}'
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (2.7.0-pre.180) ===" "Cyan"
