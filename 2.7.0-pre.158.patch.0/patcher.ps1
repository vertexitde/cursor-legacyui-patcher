<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 2.7.0-pre.158 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 2.7.0-pre.158.
    pre.158 variables: HI_ (2 layouts), Eh (clear), w2 (layoutEnum), AM (cmd), jI_ (createInstance), pi (StorageScope), openerCheck WD.
    Note: Uses C() for i18n; Zen label C(3186)=Zen Mode (not C(3065) Layout+Zen). Browser C(3066).
    Stock buildContent: glass button may appear before settings; expanded Replace keeps appendGlassWindowButton(n) last.
    Opener: getAnchor ()=>$, render U=>, createInstance jI_.
#>
...
$Patches = @(
    ...
    @{
        ...
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>$,anchorAlignment:1,anchorPosition:0,layer:-24,render:U=>{try{const G=WD(a,u)?_:void 0;return i.createInstance(jI_'
        Replace = '{getAnchor:()=>$,anchorAlignment:1,anchorPosition:0,render:U=>{try{const G=WD(a,u)?_:void 0;return i.createInstance(jI_'
    },
    @{
        Name                = "buildContent_full_pre158"
        AllowLengthMismatch = $true
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (2.7.0-pre.158) ===" "Cyan"
