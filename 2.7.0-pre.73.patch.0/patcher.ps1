<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 2.7.0-pre.73 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 2.7.0-pre.73.
    pre.73 variables: c7w (2 layouts), dh (clear), fD (layoutEnum), SN (cmd), m7w (createInstance), xi (StorageScope), qD (openerCheck).
    Note: Uses C() for i18n, getSidebarLocation() in renderModeGrid.
    NLS IDs: Agent=2952, Editor=2953, Settings=2951, Zen=3191, Browser=3192.
    isAgentLayoutActive() removed; isEmptyWindow() not in class; noInlineDiffs not in class.
    chatEditorGroupService: hasChatEditors + ensureChatVisibleOrCreate (no showChatEditorGroup).
    xi StorageScope used for editor and statusbar isVisible calls.
#>
...
$Patches = @(
    ...
    @{
        ...
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>$,anchorAlignment:1,anchorPosition:0,layer:-24,render:q=>{try{const J=qD(a,u)?_:void 0;return i.createInstance(m7w'
        Replace = '{getAnchor:()=>$,anchorAlignment:1,anchorPosition:0,render:q=>{try{const J=qD(a,u)?_:void 0;return i.createInstance(m7w'
    },
    @{
        Name                = "buildContent_full_pre73"
        AllowLengthMismatch = $true
    ...
    @{
        Name                = "appendCursorSettingsButton_keybinding_pre73"
        AllowLengthMismatch = $true
        Search  = 'appendCursorSettingsButton(n){const e=document.createElement("button");e.type="button",e.className="agent-layout-quick-menu__footer-link";const t=document.createElement("span");t.className="agent-layout-quick-menu__label",t.textContent=C(2951,null),e.appendChild(t),e.addEventListener("click",i=>{i.stopPropagation(),this.commandService.executeCommand(SN).finally(()=>this.onRequestClose())}),n.appendChild(e)}'
        Replace = 'appendCursorSettingsButton(n){const e=document.createElement("button");e.type="button",e.className="agent-layout-quick-menu__footer-link";const t=document.createElement("span");t.className="agent-layout-quick-menu__label",t.textContent="Cursor Settings",e.appendChild(t);const i=this.getKeybindingLabel(SN);if(i){const r=document.createElement("span");r.className="agent-layout-quick-menu__keybinding",r.textContent=i,e.appendChild(r)}e.addEventListener("click",o=>{o.stopPropagation(),this.commandService.executeCommand(SN).finally(()=>this.onRequestClose())}),n.appendChild(e)}'
    }
)
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (2.7.0-pre.73) ===" "Cyan"
