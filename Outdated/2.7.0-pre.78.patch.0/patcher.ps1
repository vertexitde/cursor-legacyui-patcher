<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 2.7.0-pre.78 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 2.7.0-pre.78.
    pre.78 variables: jWw (2 layouts), mh (clear), vD (layoutEnum), CN (cmd), ZWw (createInstance), Ii (StorageScope), HD (openerCheck).
    Note: Uses C() for i18n, getSidebarLocation() in renderModeGrid.
    NLS IDs: Agent=2957, Editor=2958, Settings=2956, Zen=3198, Browser=3199.
    getAnchor var changed to q (render callback is $). isAgentLayoutActive/isEmptyWindow/noInlineDiffs not in class.
#>
...
$Patches = @(
    ...
    @{
        ...
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>q,anchorAlignment:1,anchorPosition:0,layer:-24,render:$=>{try{const G=HD(a,u)?_:void 0;return i.createInstance(ZWw'
        Replace = '{getAnchor:()=>q,anchorAlignment:1,anchorPosition:0,render:$=>{try{const G=HD(a,u)?_:void 0;return i.createInstance(ZWw'
    },
    @{
        Name                = "buildContent_full_pre78"
        AllowLengthMismatch = $true
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (2.7.0-pre.78) ===" "Cyan"
