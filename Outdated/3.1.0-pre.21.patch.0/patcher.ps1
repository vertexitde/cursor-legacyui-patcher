<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 3.1.0-pre.21 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 3.1.0-pre.21.
    i18n E(). vFS (layouts), Dh (clear), eF (layoutEnum), nB (settings cmd), fi (StorageScope), opener ER + SFS.
    Zen label "Zen"; Browser E(3094) ("Browser" in nls). Agent/Editor E(3072)/E(3073). Settings E(3069).
    Opener: getAnchor ()=>O, layer:-24 removed; createInstance(SFS,...).render(U); tail N,x (pre.12 had M,x + wR/kRS).
    Add-layout tile: !isApplyingLayout && under cap (no unsaved-only gate). Minified names differ from cursor-old and from pre.12.
#>
...
# 3.1.0-pre.21: vFS=2 layouts, Dh=clear, eF=layoutEnum, nB=cmd, SFS=createInstance, fi=StorageScope, openerCheck=ER (getAnchor:()=>O)
$Patches = @(
    @{
        Name                = "getDefaultLayouts_4layouts"
        AllowLengthMismatch = $true
        Search  = 'getDefaultLayouts(){return vFS}'
        Replace = 'getDefaultLayouts(){return[...vFS,{id:"default-zen",name:"Zen",state:{agentsVisible:!1,chatVisible:!1,editorsVisible:!0,panelVisible:!1,panelMaximized:!1,sidebarVisible:!1,sidebarLocation:"left",partWidths:{}},type:"default"},{id:"default-browser",name:E(3094,null),state:{agentsVisible:!1,chatVisible:!0,editorsVisible:!0,panelVisible:!1,sidebarVisible:!1,sidebarLocation:"left",partWidths:{},panelMaximized:!1},type:"default"}]}'
    },
    @{
        Name                = "renderModeGrid_321p21"
        AllowLengthMismatch = $true
    ...
    @{
        Name                = "opener_no_layer_321p21"
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>O,anchorAlignment:1,anchorPosition:0,layer:-24,render:U=>{try{const q=ER(a,u)?S:void 0;return i.createInstance(SFS,s,o,a,l,u,d,h,f,p,g,i,_,q,N,x).render(U)}catch(q){throw q}}'
        Replace = '{getAnchor:()=>O,anchorAlignment:1,anchorPosition:0,render:U=>{try{const q=ER(a,u)?S:void 0;return i.createInstance(SFS,s,o,a,l,u,d,h,f,p,g,i,_,q,N,x).render(U)}catch(q){throw q}}'
    },
    @{
        Name                = "buildContent_full_321p21"
        AllowLengthMismatch = $true
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (3.1.0-pre.21) ===" "Cyan"
