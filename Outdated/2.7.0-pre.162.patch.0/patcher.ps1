<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 2.7.0-pre.162 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 2.7.0-pre.162.
    pre.162 variables: jD_ (2 layouts), Eh (clear), C2 (layoutEnum), yM (cmd), ZD_ (createInstance), pi (StorageScope), openerCheck zD.
    Zen C(3189)=Zen Mode (not C(3068) Layout+Zen). Browser C(3069). Agent/Editor C(3047)/C(3048). Settings footer C(3044).
    ...
    Opener: getAnchor ()=>$, render U=>, createInstance ZD_.
#>
...
$Patches = @(
    ...
    @{
        ...
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>$,anchorAlignment:1,anchorPosition:0,layer:-24,render:U=>{try{const G=zD(a,u)?_:void 0;return i.createInstance(ZD_'
        Replace = '{getAnchor:()=>$,anchorAlignment:1,anchorPosition:0,render:U=>{try{const G=zD(a,u)?_:void 0;return i.createInstance(ZD_'
    },
    @{
        Name                = "buildContent_full_pre162"
        AllowLengthMismatch = $true
    ...
    @{
        Name                = "appendCursorSettingsButton_keybinding_pre162"
        AllowLengthMismatch = $true
        Search  = 'appendCursorSettingsButton(n){const e=document.createElement("button");e.type="button",e.className="agent-layout-quick-menu__footer-link";const t=document.createElement("span");t.className="agent-layout-quick-menu__label",t.textContent=C(3044,null),e.appendChild(t),e.addEventListener("click",i=>{i.stopPropagation(),this.commandService.executeCommand(yM).finally(()=>this.onRequestClose())}),n.appendChild(e)}'
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (2.7.0-pre.162) ===" "Cyan"
