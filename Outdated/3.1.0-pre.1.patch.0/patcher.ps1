<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 3.1.0-pre.1 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 3.1.0-pre.1.
    i18n E() (this build uses E, not C). AAS (layouts), Ih (clear), eF (layoutEnum), Z2 (settings cmd), wi (StorageScope), opener AR + LAS.
    Zen E(3212), Browser E(3092); avoid E(3091) Layout+Zen. Agent/Editor E(3070)/E(3071). Settings E(3067).
    Opener: getAnchor ()=>O (letter O, not $), layer:-24 removed; createInstance(LAS,...).render(U).
#>
...
# 3.1.0-pre.1: AAS=2 layouts, Ih=clear, eF=layoutEnum, Z2=cmd, LAS=createInstance, wi=StorageScope, openerCheck=AR (anchor getAnchor:()=>O)
$Patches = @(
    @{
        Name                = "getDefaultLayouts_4layouts"
        AllowLengthMismatch = $true
        Search  = 'getDefaultLayouts(){return AAS}'
        Replace = 'getDefaultLayouts(){return[...AAS,{id:"default-zen",name:E(3212,null),state:{agentsVisible:!1,chatVisible:!1,editorsVisible:!0,panelVisible:!1,panelMaximized:!1,sidebarVisible:!1,sidebarLocation:"left",partWidths:{}},type:"default"},{id:"default-browser",name:E(3092,null),state:{agentsVisible:!1,chatVisible:!0,editorsVisible:!0,panelVisible:!1,sidebarVisible:!1,sidebarLocation:"left",partWidths:{},panelMaximized:!1},type:"default"}]}'
    },
    @{
        Name                = "renderModeGrid_310p1"
        AllowLengthMismatch = $true
    ...
    @{
        Name                = "opener_no_layer_310p1"
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>O,anchorAlignment:1,anchorPosition:0,layer:-24,render:U=>{try{const q=AR(a,u)?S:void 0;return i.createInstance(LAS,s,o,a,l,u,d,h,f,p,g,i,_,q,M,k).render(U)}catch(q){throw q}}'
        Replace = '{getAnchor:()=>O,anchorAlignment:1,anchorPosition:0,render:U=>{try{const q=AR(a,u)?S:void 0;return i.createInstance(LAS,s,o,a,l,u,d,h,f,p,g,i,_,q,M,k).render(U)}catch(q){throw q}}'
    },
    @{
        Name                = "buildContent_full_310p1"
        AllowLengthMismatch = $true
    ...
    @{
        Name                = "appendCursorSettingsButton_keybinding_310p1"
        AllowLengthMismatch = $true
        Search  = 'appendCursorSettingsButton(n){const e=document.createElement("button");e.type="button",e.className="agent-layout-quick-menu__footer-link";const t=document.createElement("span");t.className="agent-layout-quick-menu__label",t.textContent=E(3067,null),e.appendChild(t),e.addEventListener("click",i=>{i.stopPropagation(),this.commandService.executeCommand(Z2).finally(()=>this.onRequestClose())}),n.appendChild(e)}'
        Replace = 'appendCursorSettingsButton(n){const e=document.createElement("button");e.type="button",e.className="agent-layout-quick-menu__footer-link";const t=document.createElement("span");t.className="agent-layout-quick-menu__label",t.textContent="Cursor Settings",e.appendChild(t);const i=this.getKeybindingLabel(Z2);if(i){const r=document.createElement("span");r.className="agent-layout-quick-menu__keybinding",r.textContent=i,e.appendChild(r)}e.addEventListener("click",o=>{o.stopPropagation(),this.commandService.executeCommand(Z2).finally(()=>this.onRequestClose())}),n.appendChild(e)}'
    }
)
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (3.1.0-pre.1) ===" "Cyan"
