<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 2.7.0-pre.87 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 2.7.0-pre.87.
    pre.87 variables: _o_ (2 layouts), ph (clear), ED (layoutEnum), DN (cmd), xo_ (createInstance), Ci (StorageScope), zR (openerCheck).
    Note: Uses C() for i18n, getSidebarLocation() in renderModeGrid.
    NLS IDs: Agent=3025, Editor=3026, Settings=3022, Zen=3266, Browser=3267.
    getAnchor var=q, render callback=$, opener var=G.
    buildContent now ends with appendCursorSettingsButton + appendGlassWindowButton (preserve both).
...
$Patches = @(
    ...
    @{
        ...
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>q,anchorAlignment:1,anchorPosition:0,layer:-24,render:$=>{try{const G=zR(a,u)?_:void 0;return i.createInstance(xo_'
        Replace = '{getAnchor:()=>q,anchorAlignment:1,anchorPosition:0,render:$=>{try{const G=zR(a,u)?_:void 0;return i.createInstance(xo_'
    },
    @{
        Name                = "buildContent_full_pre87"
        AllowLengthMismatch = $true
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (2.7.0-pre.87) ===" "Cyan"
