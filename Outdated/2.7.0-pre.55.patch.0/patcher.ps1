<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 2.7.0-pre.55 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 2.7.0-pre.55.
    pre.55 variables: H6w (2 layouts), ch (clear), FL/j6w (opener), vL (cmd), _i (StorageScope), V0 (layoutEnum).
    Note: Uses C() for i18n, and getSidebarLocation() in renderModeGrid.
#>
...
# pre.55: H6w=2 layouts, ch=clear, FL/j6w=opener, vL=cmd, _i=StorageScope, V0=layoutEnum
$Patches = @(
    @{
        Name                = "getDefaultLayouts_4layouts"
        AllowLengthMismatch = $true
        Search  = 'getDefaultLayouts(){return H6w}'
        Replace = 'getDefaultLayouts(){return[...H6w,{id:"default-zen",name:C(3183,null),state:{agentsVisible:!1,chatVisible:!1,editorsVisible:!0,panelVisible:!1,panelMaximized:!1,sidebarVisible:!1,sidebarLocation:"left",partWidths:{}},type:"default"},{id:"default-browser",name:C(3184,null),state:{agentsVisible:!1,chatVisible:!1,editorsVisible:!1,panelVisible:!1,panelMaximized:!1,sidebarVisible:!1,sidebarLocation:"left",partWidths:{}},type:"default"}]}'
    },
    @{
        Name                = "renderModeGrid_pre55"
        AllowLengthMismatch = $true
    ...
    @{
        ...
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>$,anchorAlignment:1,anchorPosition:0,layer:-24,render:q=>{try{const J=FL(a,u)?_:void 0;return i.createInstance(j6w'
        Replace = '{getAnchor:()=>$,anchorAlignment:1,anchorPosition:0,render:q=>{try{const J=FL(a,u)?_:void 0;return i.createInstance(j6w'
    },
    @{
        Name                = "buildContent_full_pre55"
        AllowLengthMismatch = $true
    ...
    @{
        Name                = "appendCursorSettingsButton_keybinding_pre55"
        AllowLengthMismatch = $true
        Search  = 'appendCursorSettingsButton(n){const e=document.createElement("button");e.type="button",e.className="agent-layout-quick-menu__footer-link";const t=document.createElement("span");t.className="agent-layout-quick-menu__label",t.textContent=C(2939,null),e.appendChild(t),e.addEventListener("click",i=>{i.stopPropagation(),this.commandService.executeCommand(vL).finally(()=>this.onRequestClose())}),n.appendChild(e)}'
        Replace = 'appendCursorSettingsButton(n){const e=document.createElement("button");e.type="button",e.className="agent-layout-quick-menu__footer-link";const t=document.createElement("span");t.className="agent-layout-quick-menu__label",t.textContent="Cursor Settings",e.appendChild(t);const i=this.getKeybindingLabel(vL);if(i){const r=document.createElement("span");r.className="agent-layout-quick-menu__keybinding",r.textContent=i,e.appendChild(r)}e.addEventListener("click",o=>{o.stopPropagation(),this.commandService.executeCommand(vL).finally(()=>this.onRequestClose())}),n.appendChild(e)}'
    }
)
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (2.7.0-pre.55) ===" "Cyan"
