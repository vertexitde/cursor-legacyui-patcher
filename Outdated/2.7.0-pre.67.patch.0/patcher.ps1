<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 2.7.0-pre.67 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 2.7.0-pre.67.
    pre.67 variables: uqw (2 layouts), ch (clear), PD/gqw (opener), dN (cmd), ki (StorageScope), cD (layoutEnum).
    ...
    renderModeGrid no longer calls ensureAgentLayoutEnabled().
#>
...
$Patches = @(
    ...
    @{
        ...
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>$,anchorAlignment:1,anchorPosition:0,layer:-24,render:q=>{try{const J=PD(a,u)?_:void 0;return i.createInstance(gqw'
        Replace = '{getAnchor:()=>$,anchorAlignment:1,anchorPosition:0,render:q=>{try{const J=PD(a,u)?_:void 0;return i.createInstance(gqw'
    },
    @{
        Name                = "buildContent_full_pre67"
        AllowLengthMismatch = $true
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (2.7.0-pre.67) ===" "Cyan"
