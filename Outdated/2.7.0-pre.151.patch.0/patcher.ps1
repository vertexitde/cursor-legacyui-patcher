<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 2.7.0-pre.151 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 2.7.0-pre.151.
    pre.151 variables: _x_ (2 layouts), kh (clear), p2 (layoutEnum), dM (cmd), xx_ (createInstance), wi (StorageScope), openerCheck WD (minified as two chars).
    Note: Uses C() for i18n, getSidebarLocation() in renderModeGrid.
    NLS IDs: Agent=3042, Editor=3043, Settings=3039, Zen tile/name=C(3184) Zen Mode (3063 is "Layout + Zen" in bundle), Browser=3064.
...
$Patches = @(
    ...
    @{
        ...
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>U,anchorAlignment:1,anchorPosition:0,layer:-24,render:$=>{try{const G=WD(a,u)?_:void 0;return i.createInstance(xx_'
        Replace = '{getAnchor:()=>U,anchorAlignment:1,anchorPosition:0,render:$=>{try{const G=WD(a,u)?_:void 0;return i.createInstance(xx_'
    },
    @{
        Name                = "buildContent_full_pre151"
        AllowLengthMismatch = $true
    ...
    @{
        Name                = "appendCursorSettingsButton_keybinding_pre151"
        AllowLengthMismatch = $true
        Search  = 'appendCursorSettingsButton(n){const e=document.createElement("button");e.type="button",e.className="agent-layout-quick-menu__footer-link";const t=document.createElement("span");t.className="agent-layout-quick-menu__label",t.textContent=C(3039,null),e.appendChild(t),e.addEventListener("click",i=>{i.stopPropagation(),this.commandService.executeCommand(dM).finally(()=>this.onRequestClose())}),n.appendChild(e)}'
        Replace = 'appendCursorSettingsButton(n){const e=document.createElement("button");e.type="button",e.className="agent-layout-quick-menu__footer-link";const t=document.createElement("span");t.className="agent-layout-quick-menu__label",t.textContent="Cursor Settings",e.appendChild(t);const i=this.getKeybindingLabel(dM);if(i){const r=document.createElement("span");r.className="agent-layout-quick-menu__keybinding",r.textContent=i,e.appendChild(r)}e.addEventListener("click",o=>{o.stopPropagation(),this.commandService.executeCommand(dM).finally(()=>this.onRequestClose())}),n.appendChild(e)}'
    }
)
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (2.7.0-pre.151) ===" "Cyan"
