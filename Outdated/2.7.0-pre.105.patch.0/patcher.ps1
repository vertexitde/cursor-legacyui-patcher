<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 2.7.0-pre.105 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 2.7.0-pre.105.
    pre.105 variables: Vu_ (2 layouts), Ah (clear), OD (layoutEnum), JN (cmd), ed_ (createInstance), yi (StorageScope), dB (openerCheck).
...
$Patches = @(
    ...
    @{
        ...
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>q,anchorAlignment:1,anchorPosition:0,layer:-24,render:$=>{try{const G=dB(a,u)?_:void 0;return i.createInstance(ed_'
        Replace = '{getAnchor:()=>q,anchorAlignment:1,anchorPosition:0,render:$=>{try{const G=dB(a,u)?_:void 0;return i.createInstance(ed_'
    },
    @{
        Name                = "buildContent_full_pre105"
        AllowLengthMismatch = $true
    ...
    @{
        Name                = "appendCursorSettingsButton_keybinding_pre105"
        AllowLengthMismatch = $true
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (2.7.0-pre.105) ===" "Cyan"
