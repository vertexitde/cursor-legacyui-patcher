<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 3.1.0-pre.12 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 3.1.0-pre.12.
    i18n E(). yRS (layouts), Eh (clear), tF (layoutEnum), X2 (settings cmd), wi (StorageScope), opener wR + kRS.
    Zen tile label "Zen" (not E(3093)—that string is "Layout • Zen" in nls). Browser E(3094) or literal "Browser". Agent/Editor E(3072)/E(3073). Settings E(3069).
    Opener: getAnchor ()=>O (letter O, not $), layer:-24 removed; createInstance(kRS,...).render(U); tail args M,x.
...
$Patches = @(
    ...
    @{
        Name                = "opener_no_layer_312p12"
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>O,anchorAlignment:1,anchorPosition:0,layer:-24,render:U=>{try{const q=wR(a,u)?S:void 0;return i.createInstance(kRS,s,o,a,l,u,d,h,f,p,g,i,_,q,M,x).render(U)}catch(q){throw q}}'
        Replace = '{getAnchor:()=>O,anchorAlignment:1,anchorPosition:0,render:U=>{try{const q=wR(a,u)?S:void 0;return i.createInstance(kRS,s,o,a,l,u,d,h,f,p,g,i,_,q,M,x).render(U)}catch(q){throw q}}'
    },
    @{
        Name                = "buildContent_full_312p12"
        AllowLengthMismatch = $true
    ...
    @{
        Name                = "appendCursorSettingsButton_keybinding_312p12"
        AllowLengthMismatch = $true
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (3.1.0-pre.12) ===" "Cyan"
