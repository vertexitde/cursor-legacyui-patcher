<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 2.7.0-pre.148 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 2.7.0-pre.148.
    pre.148 variables: VE_ (2 layouts), Eh (clear), h2 (layoutEnum), oM (cmd), ex_ (createInstance), Ai (StorageScope), openerCheck symbol $D (minified as two chars).
    Note: Uses C() for i18n, getSidebarLocation() in renderModeGrid.
    NLS IDs: Agent=3033, Editor=3034, Settings=3030, Zen=3277, Browser=3278.
...
$Patches = @(
    ...
    @{
        ...
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>U,anchorAlignment:1,anchorPosition:0,layer:-24,render:$=>{try{const G=$D(a,u)?_:void 0;return i.createInstance(ex_'
        Replace = '{getAnchor:()=>U,anchorAlignment:1,anchorPosition:0,render:$=>{try{const G=$D(a,u)?_:void 0;return i.createInstance(ex_'
    },
    @{
        Name                = "buildContent_full_pre148"
        AllowLengthMismatch = $true
    ...
    @{
        Name                = "appendCursorSettingsButton_keybinding_pre148"
        AllowLengthMismatch = $true
        Search  = 'appendCursorSettingsButton(n){const e=document.createElement("button");e.type="button",e.className="agent-layout-quick-menu__footer-link";const t=document.createElement("span");t.className="agent-layout-quick-menu__label",t.textContent=C(3030,null),e.appendChild(t),e.addEventListener("click",i=>{i.stopPropagation(),this.commandService.executeCommand(oM).finally(()=>this.onRequestClose())}),n.appendChild(e)}'
        Replace = 'appendCursorSettingsButton(n){const e=document.createElement("button");e.type="button",e.className="agent-layout-quick-menu__footer-link";const t=document.createElement("span");t.className="agent-layout-quick-menu__label",t.textContent="Cursor Settings",e.appendChild(t);const i=this.getKeybindingLabel(oM);if(i){const r=document.createElement("span");r.className="agent-layout-quick-menu__keybinding",r.textContent=i,e.appendChild(r)}e.addEventListener("click",o=>{o.stopPropagation(),this.commandService.executeCommand(oM).finally(()=>this.onRequestClose())}),n.appendChild(e)}'
    }
)
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (2.7.0-pre.148) ===" "Cyan"
