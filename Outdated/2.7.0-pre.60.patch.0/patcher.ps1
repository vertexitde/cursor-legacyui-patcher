<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 2.7.0-pre.60 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 2.7.0-pre.60.
    pre.60 variables: iUw (2 layouts), ah (clear), RL/cUw (opener), gL (cmd), _i (StorageScope), Q0 (layoutEnum).
    Note: Uses C() for i18n, getSidebarLocation() in renderModeGrid.
    NLS IDs: Agent=2943, Editor=2944, Settings=2942, Zen=3186, Browser=3187.
#>
...
$Patches = @(
    ...
    @{
        ...
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>$,anchorAlignment:1,anchorPosition:0,layer:-24,render:q=>{try{const G=RL(a,u)?_:void 0;return i.createInstance(cUw'
        Replace = '{getAnchor:()=>$,anchorAlignment:1,anchorPosition:0,render:q=>{try{const G=RL(a,u)?_:void 0;return i.createInstance(cUw'
    },
    @{
        Name                = "buildContent_full_pre60"
        AllowLengthMismatch = $true
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (2.7.0-pre.60) ===" "Cyan"
