<#
.SYNOPSIS
    Cursor Layout Menu Patcher for 2.7.0-pre.176 - Restores the full "Change Layout" menu.

.DESCRIPTION
    Patches workbench.desktop.main.js for Cursor 2.7.0-pre.176.
    i18n E(). Q0S (layouts), Th (clear), zB (layoutEnum), Y2 (cmd), nCS (createInstance), Si (StorageScope), opener bR.
    Zen E(3204), Browser E(3084); avoid E(3083) Layout+Zen. Agent/Editor E(3062)/E(3063). Settings E(3059).
    Opener: getAnchor ()=>O, const q=bR, createInstance nCS (removes layer:-24).
#>
...
$Patches = @(
    ...
    @{
        ...
        AllowLengthMismatch = $true
        Search  = '{getAnchor:()=>O,anchorAlignment:1,anchorPosition:0,layer:-24,render:U=>{try{const q=bR(a,u)?w:void 0;return i.createInstance(nCS'
        Replace = '{getAnchor:()=>O,anchorAlignment:1,anchorPosition:0,render:U=>{try{const q=bR(a,u)?w:void 0;return i.createInstance(nCS'
    },
    @{
        Name                = "buildContent_full_pre176"
        AllowLengthMismatch = $true
    ...
    @{
        Name                = "appendCursorSettingsButton_keybinding_pre176"
        AllowLengthMismatch = $true
        Search  = 'appendCursorSettingsButton(n){const e=document.createElement("button");e.type="button",e.className="agent-layout-quick-menu__footer-link";const t=document.createElement("span");t.className="agent-layout-quick-menu__label",t.textContent=E(3059,null),e.appendChild(t),e.addEventListener("click",i=>{i.stopPropagation(),this.commandService.executeCommand(Y2).finally(()=>this.onRequestClose())}),n.appendChild(e)}'
...
function Invoke-Patch {
    Write-ColorOutput "=== Cursor Layout Patcher (2.7.0-pre.176) ===" "Cyan"
